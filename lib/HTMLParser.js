...
function HTMLParser(address, fragmentContext, options) {
  ...
  function createHTMLElt(doc, name, attrs) {
    ...
    // Should these be reworked to go in the for-loop below? It would be uglier but faster.
    if (options && options.removeElements && options.removeElements(name, attrs)) {
      // Mark for removal.
      attrs = [ ['class', options.removalClassName] ];
    } else if (options && options.addClasses) {
      attrs = options.addClasses(name, attrs);
    }
    ...
    if (attrs) {
      for(var i = 0, n = attrs.length; i < n; i++) {
        if (options
            ...
            && options.removeAttributes(name, attrs[i][0], attrs[i][1])) {
          // Omit this attribute.
          continue;
        }
        if (options && options.updateAttributes
            && options.updateAttributes[name, attrs[i][0]]) {
          attrs[i][1] = options.updateAttributes[name, attrs[i][0]](attrs[i][1]);
        }
